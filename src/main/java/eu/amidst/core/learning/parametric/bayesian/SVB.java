...
public class SVB implements BayesianParameterLearningAlgorithm, Serializable {
    ...
    public void setMaxIterations(int maxIterations){
        this.plateuStructure.getVMP().setMaxIter(maxIterations);
    }

    public void setThresold(double thresold){
        this.plateuStructure.getVMP().setThreshold(thresold);
    }
    ...
    protected CompoundVector computeNaturalParameterVectorPrior(){
        return this.getPlateuStructure().getParameterVariablePriors();
    }
    ...
    public double predictedLogLikelihood(DataOnMemory<DataInstance> batch) {
        this.getPlateuStructure().getNonReplicatedNodes().forEach(node -> node.setActive(false));

        double elbo = 0;// this.getPlateuStructure().getNonReplicatedNodes().mapToDouble(node -> this.getPlateuStructure().getVMP().computeELBO(node)).sum();

        elbo += this.updateModelOnBatchParallel(batch).getElbo();

        this.getPlateuStructure().getNonReplicatedNodes().forEach(node -> node.setActive(true));
    ...
    public BatchOutput updateModelOnBatchParallel(DataOnMemory<DataInstance> batch) {
        ...
        CompoundVector compoundVectorEnd = this.plateuStructure.getParameterVariablesPosterior();
    ...
    public void randomInitialize(){
        ...
        CompoundVector posterior = this.getPlateuStructure().getParameterVariablesPosterior();

        this.getPlateuStructure().setVmp(original);

        this.initLearning();

        this.plateuStructure.updateParameterVariablesPosterior(posterior);
    }
     ...
     */
    /* MyNote: Se le pasa un vector de parametros (naturales o de momento) correspondientes a las posteriors */
    public void updateNaturalParameterPrior(CompoundVector parameterVector){
        this.plateuStructure.updateParameterVariablesPrior(parameterVector);
        this.ef_extendedBN = this.plateuStructure.getEFLearningBN();
    ...
    public void updateNaturalParameterPosteriors(CompoundVector parameterVector){
        this.plateuStructure.updateParameterVariablesPosterior(parameterVector);
    }
    /**
     * {@inheritDoc}
     */
