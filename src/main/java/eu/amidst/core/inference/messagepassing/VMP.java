...
public class VMP extends MessagePassingAlgorithm<NaturalParameters> implements InferenceAlgorithm, Sampler {
     ...
     */
    @Override
    public boolean testConvergenceHC(List<Node> nodes) {

        boolean convergence = false;

        //Compute lower-bound
        double newelbo = this.computeLogProbabilityOfEvidenceHC(nodes);
    ...
    public double computeLogProbabilityOfEvidenceHC(List<Node> nodes) {
        return nodes.stream().filter(node -> node.isActive()).mapToDouble(node -> this.computeELBO(node)).sum();
    }

    public Map<Variable, Double> computeLogProbabilityOfEvidenceMap() {

        Map<Variable, Double> scores = new HashMap<>();
        for(Variable variable: this.variablesToNode.keySet())
            scores.put(variable, 0.0);

        // TODO: Faltaria calcular el score de los nodos no replicados (seleccionamos variables no-parameter unicamente)

        for(Node node: this.nodes){
            double score = scores.get(node.getMainVariable());
            score += computeELBO(node);
            scores.put(node.getMainVariable(), score);
        }
