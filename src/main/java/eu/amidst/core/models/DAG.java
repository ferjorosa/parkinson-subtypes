 ...
 *
 */
// TODO: Sustituiria esta clase por el DirectedAcyclicGraph de Voltric que es mucho mejor y mas completo
public class DAG implements Serializable {
    ...
    private LinkedHashMap<Variable, ParentSet> parents;
    ...
    public DAG(Variables variables) {
        ...
        for (Variable var : variables) {
            parents.put(var, new ParentSetImpl(var));
        }
        //this.parents = Collections.unmodifiableList(parents);
        this.variables.block();
    }

    /** Copy constructor. It will be called by deepCopy() */
    public DAG(DAG dag, Variables variables) {
        if(!dag.getVariables().equals(variables))
            throw new IllegalArgumentException("the argument Variables must equal the DAG's internal Variables object");

        this.name = dag.getName();
        this.variables = variables;
        this.parents = new LinkedHashMap<>();

        /* Iteramos por el conjunto de variables nuevo*/
        for(Variable var: variables) {
            ParentSetImpl parentSetCopy = new ParentSetImpl(var);

            /* Copiamos el parent Set con la nueva variable */
            for(Variable parent: dag.getParentSet(var)){
                Variable parentCopy = variables.getVariableByName(parent.getName());
                parentSetCopy.addParent(parentCopy);
            }

            this.parents.put(var, parentSetCopy);
        }
        this.variables.block();
    }
    ...
    public long getNumberOfLinks(){
        return this.parents.values().stream().mapToInt(p -> p.getNumberOfParents()).sum();
    }
    ...
    public List<ParentSet> getParentSets() {
        return new ArrayList<>(this.parents.values());
    }
    ...
    //TODO: Ineficient given that a variable doesnt have children. It requires a revamp, combining Amidst and Voltric
    public void dfs(Variable variable, Map<Variable, Integer> visitedVariables) {

        visitedVariables.put(variable, 1); // discovers the argument variable

        // explores unvisited children
        for (Variable child: GraphUtilsAmidst.getChildren(variable, this)) {
            if (!visitedVariables.containsKey(child)) {
                dfs(child, visitedVariables);
            }
        }
    }

    public void bfs(Variable variable, Map<Variable, Integer> visitedVariables) {

        visitedVariables.put(variable, 1); // discovers the argument variable. It is only here for the root, because it will rediscover each nodes several times

        // explores unvisited children
        for (Variable child: GraphUtilsAmidst.getChildren(variable, this)) {
            if (!visitedVariables.containsKey(child)) {
                visitedVariables.put(child, 1);
                bfs(child, visitedVariables);
            }
        }
    }
    ...
    /**
     * Creates a deep copy with the specified Variables object (it should be equal to the DAG's internal one)
     * @param variables
     * @return
     */
    public DAG deepCopy(Variables variables) {
        return new DAG(this, variables);
    }

    /**
     * Metodo para añadir una variable al grafo.
     *
     * Nota: Simplemente añade un ParentSet con la variable en cuestión
     */
    public void addVariable(Variable variable) {
        ParentSetImpl newParentSet = new ParentSetImpl(variable);
        this.parents.put(variable, newParentSet);
    }

    /**
     * Metodo para eliminar la variable del grafo
     *
     * Nota: Simplemente eliminar el parentSet asociado
     */
    ...
    private static final class ParentSetImpl implements ParentSet, Serializable {
        ...
        */
        ...
        public boolean equals(Object o) {
            ...
            ParentSetImpl parentSet = (ParentSetImpl) o;
            return Objects.equals(mainVar, parentSet.mainVar);
        }
