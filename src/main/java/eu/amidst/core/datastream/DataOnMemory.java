...
public interface DataOnMemory<E extends DataInstance> extends DataStream<E> {
     ...
     */
    default DataOnMemory<DataInstance> deepCopy(){
        Attributes copyAttributes = this.getAttributes().deepCopy();
        List<DataInstance> dataInstanceList = new ArrayList<>();

        for(DataInstance instance: this){
            double[] data = new double[copyAttributes.getNumberOfAttributes()];
            for (int i = 0; i < copyAttributes.getNumberOfAttributes(); i++) {
                data[i] = instance.getValue(copyAttributes.getFullListOfAttributes().get(i));
            }
            dataInstanceList.add(new DataInstanceFromRawData(copyAttributes, data));
        }

        return new DataOnMemoryListContainer<>(copyAttributes, dataInstanceList);
    }
}
