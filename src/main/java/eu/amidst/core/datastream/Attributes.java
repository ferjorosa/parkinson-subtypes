...
public class Attributes implements Serializable, Iterable<Attribute> {
    ...
    public Attributes deepCopy() {
        ...
        List<Attribute> copyAttributeList = new ArrayList<>();

        for(Attribute attribute: this.getFullListOfAttributes())
            copyAttributeList.add(new Attribute(attribute.getIndex(), attribute.getName(), attribute.getStateSpaceType()));

        return new Attributes(copyAttributeList);
    }
}